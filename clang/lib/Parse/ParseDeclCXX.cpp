                                         ...
                                         SourceLocation *EndLoc)
{
  ...
  if (ParseSpliceSpecifier(false)) {
    Diag(Tok.getLocation(), diag::p3385_err_attribute_splicing_error)
      << "Error parsing the splice specifier";
    return true;
  }

  //    constexpr auto r = ^^[[nodiscard]];
  //    enum class [[ [: r :] ]] E {}; // <= No parsaran as Expr
  ArgsVector ArgExprs;
  ParsedAttr* recoveredAttr = nullptr;
  if (!ParseAttributeReflection(recoveredAttr)) {
    if (recoveredAttr->getNumArgs() != 0) {
      Diag(Tok.getLocation(), diag::p3385_trace_execution_checkpoint)
        << "Found argument while splicing a reflected attribute";
      ArgExprs.push_back(recoveredAttr->getArg(0));
    }
    Attrs.addNew(
        const_cast<IdentifierInfo*>(recoveredAttr->getAttrName()),
        range,
        nullptr, Tok.getLocation(), ArgExprs.data(), ArgExprs.size(),
        ParsedAttr::Form::CXX11());
    // Note that ParseAttributeReflection does not consume the token
    // So that later ParseCXXSpliceAsExpr can work as is w/o reinjecting
    // a token, so we consume it here
    ConsumeAnnotationToken();
    return false;
  }

  ExprResult Result = ParseCXXSpliceAsExpr(SourceLocation(), false);

  if(Result.isInvalid()) {
    Diag(Tok.getLocation(), diag::p3385_err_attribute_splicing_error)
    << "Error parsing the splice specifier as an expression";
  }

  // We re back to a splice-specifier that isn't naming the reflection of
  // an attribute, so we'll recover the expression.
  auto *SpliceExpr = cast<CXXSpliceExpr>(Result.get());

  // For value dependent expression, EvaluateAsRValue die... so we ll smuggle
  // the expression in [[clang::DelayedSplice(expr)]] and we'll evaluate it
  // at instantiation
  if (SpliceExpr->isValueDependent()) {
    ...
    Attrs.addNew(
      ...
      nullptr, Tok.getLocation(), ArgExprs.data(), ArgExprs.size(),
      ParsedAttr::Form::CXX11());
    return true; // Caller will see it as an error, why did I do that ?
  }
  Expr::EvalResult ER;
  if (!SpliceExpr->EvaluateAsRValue(ER, Actions.getASTContext(), true)) {
    return false; // Really ?... Also don't know why I did that
  }

  assert(ER.Val.getKind() == APValue::Reflection);
  switch (ER.Val.getReflectionKind()) {
    case ReflectionKind::Attribute: {
      // We should not be here as the splice of attributes was done earlier
      Diag(Tok.getLocation(), diag::p3385_err_attribute_splicing_error)
        << "Error parsing the splice specifier as an expression";
      return true;
    }
    ...
    default:
      Diag(Tok.getLocation(), diag::p3385_err_attribute_splicing_error)
        << "Only reflection of 'attribute' or 'type' is supported in attribute splicing";
    return true;
  }
  return false;
}
...
void Parser::ParseCXX11AttributeSpecifierInternal(ParsedAttributes &Attrs,
  ...
  // FIXME Remove inplace splicing after we re done with rebase...
  //
