...
enum class Singleton : int;
consteval {
  define_enum(^^Singleton, {
...
consteval {
  define_enum(^^Empty, std::vector<info>{});
}
...
namespace is_enumerator_spec_tests {

static_assert(is_enumerator_spec(enumerator_spec({.name = "X"})));
static_assert(!is_enumerator_spec(^^int));
static_assert(!is_enumerator_spec(info{}));
...
constexpr auto rw = static_cast<Flags>(
    static_cast<unsigned>(Flags::Read) | static_cast<unsigned>(Flags::Write));
static_assert(static_cast<unsigned>(rw) == 3);
...
static_assert(identifier_of(^^Dir::East) == "East");
                    ...
                    // ==============================================
                    // templated: define_enum inside a class template
                    // ==============================================
